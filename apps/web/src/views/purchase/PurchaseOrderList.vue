  ...
  <div class="po-list">
    <div class="gs-list-toolbar">
      <div class="gs-toolbar-left">
        <el-input v-model="filters.keyword" placeholder="搜索单号 / 供应商编码 / 供应商名称" clearable :prefix-icon="SearchIcon" style="width: 280px" @keyup.enter="applyFilters" @clear="applyFilters" />
      ...
      <div class="gs-toolbar-right">
        ...
        <el-button type="primary" @click="onNew"><el-icon><Plus /></el-icon>新建采购订单</el-button>
      </div>
    </div>
        ...
        <el-form label-width="90px" label-position="right">
          <div class="gs-filter-grid">
            <el-form-item label="供应商">
              <el-select v-model="filters.supplierId" filterable remote clearable reserve-keyword placeholder="搜索真实启用供应商" :remote-method="searchSuppliers" :loading="supplierLoading" style="width:100%" @change="applyFilters">
      ...
      <el-table ref="tableRef" v-loading="store.loading" :data="visibleRows" border stripe height="100%" size="small" row-key="id" :header-cell-style="{ padding: '0 8px' }" :cell-style="{ padding: '0 8px' }" @row-dblclick="openDetail" @selection-change="onSelectionChange">
        ...
        <template #empty>
          <div class="empty-state">
            ...
            <p>{{ loadFailed ? '采购订单加载失败' : '暂无真实采购订单数据' }}</p>
            <el-button v-if="!loadFailed" type="primary" @click="onNew"><el-icon><Plus /></el-icon>新建第一张采购订单</el-button>
          </div>
        </template>
      </el-table>
    </div>
...
import { listPurchaseOrderSuppliers } from '../../api/purchase-order-context';
import { useTabsStore } from '../../stores/tabs';
import { usePurchaseOrderStore } from '../../stores/purchase-order';
import type { BusinessPartner } from '../../types/mdm';
import type { ListColumnConfig, PurchaseOrderListItemDto, PurchaseOrderStatus } from '../../types/purchase-order';
import { PURCH_STATUS_LABEL, PURCH_STATUS_TAG } from '../../types/purchase-order';
...
const selectedRows = ref<PurchaseOrderListItemDto[]>([]);
const suppliers = ref<BusinessPartner[]>([]);
const supplierLoading = ref(false);
const loadFailed = ref(false);
const filters = reactive<{ keyword: string; status?: PurchaseOrderStatus; supplierId?: string; orderDateFrom?: string; orderDateTo?: string }>({ keyword: '' });

const defaultColumns: ListColumnConfig[] = [
  { prop: 'orderNo', label: '单据号', width: 170, visible: true, fixed: 'left' },
  { prop: 'supplierNameSnapshot', label: '供应商', width: 220, visible: true },
  { prop: 'orderDate', label: '订单日期', width: 120, visible: true },
  { prop: 'expectedDeliveryDate', label: '预计交期', width: 120, visible: true },
...
const activeFilterCount = computed(() => [filters.keyword, filters.status, filters.supplierId, filters.orderDateFrom, filters.orderDateTo].filter(v => v !== undefined && v !== '').length);
const visibleRows = computed(() => filters.supplierId ? store.items.filter(r => r.supplierId === filters.supplierId) : store.items);
...
async function load() {
  ...
  try {
    ...
    ElMessage.error((err as any)?.detail || (err as any)?.title || '采购订单加载失败');
  }
}
async function searchSuppliers(keyword: string) {
  supplierLoading.value = true;
  try {
    // G3-R2B: switch from /api/v1/mdm/business-partners (which
    // requires MdmPolicies.BusinessPartnerRead that PURCH_OPERATOR
    // does not have) to the PurchaseOrder context facade endpoint
    // at /api/v1/purchase/orders/context/suppliers (which requires
    // only PurchaseOrderRead — preserves the G3-R1C boundary
    // contract).
    ...
    ElMessage.error((err as any)?.detail || (err as any)?.title || '供应商搜索失败');
  } finally {
...
function onSelectionChange(rows: PurchaseOrderListItemDto[]) { selectedRows.value = rows; }
...
function openDetail(row: PurchaseOrderListItemDto) { tabs.openDetail(row.id, row.orderNo); router.push(`/purchase-orders/${row.id}`).catch(() => {}); }
function openEdit(row: PurchaseOrderListItemDto) { tabs.openEdit(row.id, row.orderNo, 'edit'); router.push(`/purchase-orders/${row.id}/edit`).catch(() => {}); }
function onNew() { tabs.openEdit('new', '新建采购订单', 'create'); router.push('/purchase-orders/new/edit').catch(() => {}); }
function indexMethod(idx: number) { return (store.page - 1) * store.pageSize + idx + 1; }
function renderCell(prop: string, row: PurchaseOrderListItemDto) {
  switch (prop) {
    case 'orderNo': return h('a', { class: 'cell-link', onClick: (e: Event) => { e.stopPropagation(); openDetail(row); } }, row.orderNo);
    case 'supplierNameSnapshot': return h('span', { title: row.supplierCodeSnapshot }, row.supplierNameSnapshot);
    case 'status': return h(ElTag, { type: PURCH_STATUS_TAG[row.status], effect: 'light', size: 'small' }, () => PURCH_STATUS_LABEL[row.status]);
    case 'totalNetAmount':
    case 'totalTaxAmount':
...
onMounted(() => { tabs.setActive('list-purchase-order'); searchSuppliers(''); load(); });
</script>

<style scoped>
.po-list { display: flex; flex-direction: column; height: 100%; background: var(--bg-container); }
