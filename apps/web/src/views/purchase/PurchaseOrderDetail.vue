<template>
  <div v-if="order" class="po-detail" v-loading="loading">
    <div class="detail-headerbar">
      <div class="headerbar-left">
        ...
        <div class="doc-meta">
          ...
          <span class="doc-type">普通采购</span>
        </div>
        <div class="status-tags">
          <el-tag :type="PURCH_STATUS_TAG[order.status]" size="small" effect="dark">{{ PURCH_STATUS_LABEL[order.status] }}</el-tag>
    ...
    <div class="detail-section">
      <div class="section-title"><el-icon><Document /></el-icon><span>采购头信息</span></div>
      <el-descriptions :column="4" border size="small">
        ...
        <el-descriptions-item label="预计交期">{{ order.expectedDeliveryDate || '无' }}</el-descriptions-item>
        <el-descriptions-item label="单据类型">普通采购</el-descriptions-item>
        <el-descriptions-item label="供应商" :span="2"><span class="link">{{ order.supplierNameSnapshot }}</span><span class="mono small">({{ order.supplierCodeSnapshot }})</span></el-descriptions-item>
        ...
        <el-descriptions-item label="修改时间">{{ order.modifiedAt }}</el-descriptions-item>
        <el-descriptions-item label="备注" :span="4">{{ order.remarks || '无' }}</el-descriptions-item>
      </el-descriptions>
    </div>
  ...
  <el-empty v-else description="未找到该采购订单" />
</template>
...
const auditRows = computed(() => order.value ? [
  { action: 'Create/Update', actor: 'System', at: order.value.modifiedAt, remark: '操作日志接入完整流程后逐步提供' },
] : []);
...
function goBack() { tabs.setActive('list-purchase-order'); router.push('/purchase/orders').catch(() => {}); }
function edit() { if (!order.value) return; tabs.openEdit(order.value.id, order.value.orderNo, 'edit'); router.push(`/purchase-orders/${order.value.id}/edit`).catch(() => {}); }
async function confirm() {
  if (!order.value) return;
  await ElMessageBox.confirm('确认后订单及明细行将不可修改，是否继续？', '确认采购订单');
  try {
...
async function cancel() {
  if (!order.value) return;
  await ElMessageBox.confirm('确认取消该采购订单？', '取消采购订单', { type: 'warning' });
  try {
...
function printUnsupported() { ElMessage.info('打印将接入正式模板任务，当前不生成本地假文件'); }
onMounted(load);
</script>

<style scoped>
.po-detail { display: flex; flex-direction: column; height: 100%; background: #f6f7fb; overflow: auto; }
