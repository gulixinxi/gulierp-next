...
<#
G3-R2B PurchaseOrder - Web (Vite dev / frontend) evidence script.

Validates the FRONTEND half of the PurchaseOrder rebuild:
  - The Vite dev server (default :5173) serves the
    /purchase/orders routes with a 200 status.
  - The PurchaseOrderList + PurchaseOrderEdit + PurchaseOrderDetail
    files do NOT import from mock/sales-order.ts or any other mock.
  - The PurchaseOrderList + PurchaseOrderEdit files DO import
    the new purchase-order-context.ts (the G3-R2B facade).
...
ENVIRONMENT VARIABLES (all optional):
  ...
  GULIERP_G3R2B_PURCH_OPERATOR_PASS   (for Level 3 cross-check)
#>
...
$RequiredFiles = @(
    'apps/web/src/api/purchase-order.ts',
    'apps/web/src/api/purchase-order-context.ts',
    'apps/web/src/stores/purchase-order.ts',
    'apps/web/src/views/purchase/PurchaseOrderList.vue',
    'apps/web/src/views/purchase/PurchaseOrderEdit.vue',
    'apps/web/src/views/purchase/PurchaseOrderDetail.vue',
    'apps/web/src/router.ts',
    'apps/web/src/types/purchase-order.ts'
)
$missing = @()
...
if ($missing.Count -gt 0) {
    ...
    $msg = ('All {0} required PurchaseOrder files present' -f $cnt)
    Assert-Pass $msg
}

# 2b) NO mock imports in the 3 PurchaseOrder pages
$PurchaseOrderPages = @('PurchaseOrderList','PurchaseOrderEdit','PurchaseOrderDetail')
$mockViolations = @()
foreach ($page in $PurchaseOrderPages) {
    $path = ('apps/web/src/views/purchase/{0}.vue' -f $page)
...
if ($mockViolations.Count -gt 0) {
    $msg = ('Mock imports found in PurchaseOrder pages: {0}' -f ($mockViolations -join ', '))
    Assert-Fail $msg
} else {
    Assert-Pass 'No mock imports in any of the 3 PurchaseOrder pages'
}

# 2c) The 2 main pages DO import the new context facade
$listContent = Get-Content 'apps/web/src/views/purchase/PurchaseOrderList.vue' -Raw
$editContent = Get-Content 'apps/web/src/views/purchase/PurchaseOrderEdit.vue' -Raw
$listHasFacade = $listContent -match "from\s+['\`"]\.\./\.\./api/purchase-order-context"
$editHasFacade = $editContent -match "from\s+['\`"]\.\./\.\./api/purchase-order-context"
if ($listHasFacade) {
    Assert-Pass 'PurchaseOrderList imports the G3-R2B context facade'
} else {
    Assert-Fail 'PurchaseOrderList does NOT import the G3-R2B context facade'
}
if ($editHasFacade) {
    Assert-Pass 'PurchaseOrderEdit imports the G3-R2B context facade'
} else {
    Assert-Fail 'PurchaseOrderEdit does NOT import the G3-R2B context facade'
}
...
foreach ($page in $PurchaseOrderPages) {
    ...
    if ($page -in @('PurchaseOrderList','PurchaseOrderEdit') -and $hits.Count -gt 0) {
...
# 2e) The navigation.ts has a real "采购订单" entry (not the placeholder)
$navContent = Get-Content 'apps/web/src/layout/navigation.ts' -Raw
$navHasRealPurch = $navContent -match "/purchase/orders"
if ($navHasRealPurch) {
    Assert-Pass 'navigation.ts has a real 采购订单 entry pointing to /purchase/orders'
} else {
    Assert-Fail 'navigation.ts does NOT have a real 采购订单 entry'
}

# 2f) Frontend build artifact check
...
# ----------------------------------------------------------------------
# Level 3 - API + page cross-check (with PURCH_OPERATOR)
# ----------------------------------------------------------------------
Write-Section 'Level 3: API + page cross-check (PURCH_OPERATOR login + dropdown)'
...
if ($apiUp -and [System.Environment]::GetEnvironmentVariable('GULIERP_G3R2B_PURCH_OPERATOR_PASS')) {
    ...
    $body = @{
        ...
        password = [System.Environment]::GetEnvironmentVariable('GULIERP_G3R2B_PURCH_OPERATOR_PASS')
    } | ConvertTo-Json
    try {
        ...
        foreach ($p in @('suppliers','items','uoms','warehouses','payment-methods')) {
            $r = Invoke-WebRequest -Uri ($BaseUrl + '/api/v1/purchase/orders/context/' + $p + '?pageSize=1') -WebSession $sess -ErrorAction Stop
            if ($r.StatusCode -eq 200) {
        ...
        Assert-Fail ('PURCH_OPERATOR cross-check failed: {0}' -f $_.Exception.Message.Split([Environment]::NewLine)[0])
    }
} else {
    Assert-Block 'PURCH_OPERATOR: env missing or API down; cross-check skipped'
}
...
Write-Host ''
Write-Host '   1. Open http://localhost:5173/login, sign in as g3r2b_purch_operator.'
Write-Host '   2. Navigate to /purchase/orders. Expect: order list with 0 pre-existing orders.'
Write-Host '   3. Click 新建采购订单. Expect: redirect to /purchase-orders/new/edit with an empty form.'
Write-Host '   4. In the supplier dropdown, type a supplier name. Expect: dropdown populates from real MDM (suppliers only, NOT customers).'
...
Write-Host '   7. Save as draft. Expect: redirect to /purchase-orders/<id> with the new orderNo (PO-YYYYMMDD-NNNN).'
Write-Host '   8. Open DevTools Network. Confirm /api/v1/purchase/orders/* requests carry the cookie and CSRF header.'
...
if ($script:HardFailed -eq 0 -and $script:Blocked -eq 0) {
    Write-Host '  RESULT : G3_R2B_PURCHASEORDER_WEB_REBUILD_VERIFIED' -ForegroundColor Green
    exit 0
} elseif ($script:HardFailed -eq 0) {
    Write-Host '  RESULT : G3_R2B_PURCHASEORDER_WEB_REBUILD_PARTIAL' -ForegroundColor Yellow
    Write-Host '  (No failures; some checks BLOCKED due to env or web server down)' -ForegroundColor Yellow
    exit 0
} else {
    Write-Host '  RESULT : G3_R2B_PURCHASEORDER_WEB_REBUILD_FAILED' -ForegroundColor Red
    exit 5
}
