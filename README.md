...
> **Evidence-driven delivery.** Every stage produces a PowerShell script that exercises the actual code paths end-to-end. Reports live under `docs/verification/`. Current runtime evidence: **50 / 50** — the V1 state machine correctly enforces `Draft → Cancelled` for cleanup; a separate Draft target is created so the test does not attempt the (intentionally unsupported) `Confirmed → Cancelled` transition.
...
# Sales order runtime evidence (50 / 50)
