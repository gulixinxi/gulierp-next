...
| Field | Value |
|---|---|
| Goal | **API-CONTRACT-ID-001 — Snowflake/HiLo ID Safe String Wire Contract** |
| Gate | **`API_CONTRACT_ID_001_VERIFIED`** |
| Status | **CODE_READY + MDM002_MASTER_DATA_VERTICAL_SLICE + AGENT_PG_ENV_BLOCKED** — MDM-002 adds 3 master data entities (BusinessPartner + Warehouse + Location) to the existing MDM module scaffold, reusing the same Domain / Application / Infrastructure 3-project layout, the same `IMultiTenant` / `ICompanyScoped` Foundation contract, the same PostgreSQL HiLo (`identity.gulierp_hilo_sequence`), the same `MdmValidationException` RFC7807 mapping, the same 6-permission ASP.NET Core policy registration, and the same Service-Boundary Tenant-Isolation test pattern (MdmServiceBoundaryArchitectureTests). BusinessPartner is **tenant-scoped** (IMultiTenant, no Company in V1); Warehouse + Location are **tenant + company-scoped** (ICompanyScoped). **Code-side evidence PASS** (build 0/0, MDM Unit 67/67 PASS = MDM-001 57/57 + 4 MDM-002 service-boundary tests + 3 MDM-002 entity-contract tests + 3 MDM-002 enum-contract tests, Identity 21/21 no regression, Foundation 44/44 no regression; MDM Integration 12 tests discovered = MDM-001 10 + MDM-002 2; Model Differ clean: `dotnet ef migrations add _MDM002_Probe` produced 0-operation Up). MDM-002 Migration `20260821104254_MDM002_BusinessPartnerWarehouseLocation` adds 3 tables (gulierp_business_partner / gulierp_warehouse / gulierp_location) + 1 FK (location.warehouseId) + 6 indexes; MDM-001 migration `20260820190000_MDM001_InitializeMdmSchema` is **untouched** (verified via `git diff`). **Agent-side PG UNAVAILABLE** — 12 integration tests are Operator-required; do NOT run in agent session. The 2 new integration tests live in the existing `MdmPostgresIntegrationCollection` so they serialize against the canonical DB. |
...
| Local Evidence | `dotnet build GuliERP.slnx -c Release` → 22 projects, 0 warnings / 0 errors. `dotnet test tests/GuliERP.Mdm.Tests` → **57/57 PASS** (R3 +7, R4 +12, R6 +6, R7 +6 Service Boundary + 2 Seed walk-up). `dotnet test tests/GuliERP.Identity.Tests` → 21/21 PASS (no regression). `dotnet test tests/GuliERP.Foundation.Tests` → 44/44 PASS (no regression). `dotnet test tests/GuliERP.Mdm.IntegrationTests --list-tests` → 10 tests, EXIT 0. `dotnet ef migrations list` → EXIT 0, lists `20260820190000_MDM001_InitializeMdmSchema (Pending)`. `dotnet ef migrations add _R4_Probe2` → produced no corrective migration (proves 0 pending model changes). Operator harness `tools/dev/mdm-001-final-acceptance.ps1` (R7+R8+R9: 11-step one-shot + `-ResumeApiRuntime` switch that only re-runs Step 10 2 rounds API runtime + dual-mode prior evidence verification Mode A=MACHINE_TRX / Mode B=OPERATOR_TRANSCRIPT_BACKFILL); PowerShell harness self-test `tools/dev/mdm-001-final-acceptance-selftest.ps1` (**45/45 PASS** = R8 30 + R9 G1-G15 15); NOT executed in agent session due to PGPASSWORD env block. R9 added canonical `docs/verification/MDM_001_R7_OPERATOR_TRANSCRIPT_EVIDENCE.md` to backfill R7's missing TRX/log evidence via Operator transcript. |
...
| Next Mainline | **API-CONTRACT-ID-001 Operator Verification** (Operator re-runs the 6 master-data SPA pages with `test_operator_g2_004`; before this fix, `GET /api/v1/mdm/uoms/83727350616817740` returned 404 because the SPA round-tripped the id through a JSON number and JavaScript silently rounded it. After the fix, the wire is a JSON STRING for all snowflake ids; route binding accepts the string segment losslessly; the 6 pages should now open without 404 on the re-read endpoint). After Operator confirmation, NEXT MAINLINE is `MDM-002 Operator Runtime Acceptance` (integration tests) → `MDM-003 — TRAE MDM-002 Web Wiring`. |
| Forbidden follow-up without user authorization | WorkCenter implementation, Coding Engine implementation, Precision / Rounding implementation, Document Numbering implementation, Inventory / Sales / Purchase business API implementation, MDM-000 re-freeze, MDM-002 / MDM-003 front-end wiring (deferred to MDM-003), SalesOrder / PurchaseOrder / Inventory business API, Approval Workflow, SalesOrder Mock UX changes. |
| Non-blocking Technical Debt (closed under API-CONTRACT-ID-001) | `API_CONTRACT_ID_001_PAGE_ARITHMETIC_GUARD` — the type-only update to `apps/web/src/types/{mdm,auth}.ts` is now `string` for every id field. The 6 master-data `.vue` pages were NOT touched in this Goal (per brief). If any page does `+row.id`, `parseInt(row.id)`, or `Number(row.id)` on a snowflake, the precision bug returns. The handoff §1b documents the rule; the 6 pages are due for a follow-up audit in `MDM-002 / MDM-003 — TRAE Web Wiring`. Until that audit, the handoff's "Treat every id as an opaque `string` token" is a guidance rule, not a compiler-enforced one. |

## Previous Active Goal (superseded)

| Field | Value |
|---|---|
| Goal | **GULIERP-ENTERPRISE-BOOTSTRAP-001 — Formal Enterprise Bootstrap Foundation (P6-J Multi-tenant Role Isolation)** |
| Gate | `GULIERP_ENTERPRISE_BOOTSTRAP_001_RUNTIME_VERIFIED` |
| Status | **CLOSED at 2026-08-23** — Operator Apply + Operator Runtime Browser Smoke + Agent Code-side Verification all PASS. Operator Browser smoke confirmed: Backend Runtime PASS, Frontend Runtime PASS, Login PASS, Formal admin login PASS, Enterprise Organization PASS, MDM pages PASS, Sales Order pages PASS. Operator PostgreSQL read-only evidence confirmed: 3 system roles for `GULI` (ERP_SYSTEM_ADMIN + ERP_MDM_OPERATOR + ERP_SALES_OPERATOR), 14 business role claims (12 MDM + 2 Sales) + 8 Identity claims = 22 effective permissions, 3 active UserRoleAssignments for `admin`, composite UNIQUE `ux_gulierp_role_tenant_normalizedname (TenantId, NormalizedName)` in place, legacy GLOBAL UNIQUE `RoleNameIndex` dropped. Agent Code-side Verification (HEAD `00f0566`, 0 file modification, 0 SQL DML, 0 INSERT): 10/10 code-side audit dimensions PASS, 7/7 build PASS (0 warnings, 0 errors), 54/54 InMemory API smoke PASS via real ASP.NET Core `WebApplicationFactory<Program>` (15 AuthenticationFacts + 6 CsrfFacts + 6 OrganizationTreeEndpointFacts + 4 MdmAuthorizationRegressionFacts + 4 SalesAuthorizationRegressionFacts + 3 EnterpriseRolePackCrossTenantFacts + others ~10), 22 effective permission codes correctly registered, Cross-Tenant NormalizedName Provisioner diagnostic capture verified, Migration `20260823090247_RoleNameIndexToTenantScope` code-side PASS. Schema Repair closed the 23505 cross-tenant NormalizedName collision by converting legacy GLOBAL UNIQUE `RoleNameIndex` to per-tenant composite UNIQUE (TenantId, NormalizedName) via `867ed4d` (3 steps: preflight + DROP + NULL backfill + CREATE composite UNIQUE, single PG transaction). The Formal Enterprise Bootstrap foundation is end-to-end Runtime-functional for `GULI` / `GULI001` / `admin`. SalesOrder UI baseline discrepancy vs G1B-1 prototype is documented in `docs/verification/GULIERP_SALESORDER_UI_BASELINE_DISCREPANCY_NOTE.md` (5 HIGH + 6 MEDIUM + 3 LOW deltas, deferred to next goal). ||
| Entry Gate | `G2-003V2` (Identity Database Referential Integrity) CLOSED; Schema-repair prerequisite |
| Architecture / Decision | `docs/verification/GULIERP_ENTERPRISE_BOOTSTRAP_001_REPORT.md`; runbook `docs/verification/GULIERP_ROLE_TENANT_ISOLATION_OPERATOR_DB_UPGRADE_RUNBOOK.md` |
| Verification | Bootstrap tool `Release` build PASS (0 warnings, 0 errors); `GuliERP.Identity.Tests` 22/22; `GuliERP.Identity.Bootstrap.Tests` 64/64; `GuliERP.Api.Tests` 32/32; `GuliERP.Identity.IntegrationTests` (focused filter `EnterpriseBootstrapAndOrganizationTreeFacts|MdmAuthorizationRegressionFacts|SalesAuthorizationRegressionFacts`) 28/28; `GuliERP.Mdm.Tests` 67/67 (with `--artifacts-path $Temp` isolation); `GuliERP.Sales.Tests` 9/9 (with `--artifacts-path $Temp` isolation); new `GuliERP.Identity.IntegrationTests/EnterpriseRolePackCrossTenantFacts` 3/3; total 225/225 PASS. Operator Apply output captured in REPORT.md. |
| Scope (V1) | Formal Tenant (`83727350616817890` = GULI) + Formal Company (`83727350616817891` = GULI001) + Formal Admin User (`83727350616817894` = admin) bootstrap with 3 independent Roles (ERP_SYSTEM_ADMIN + ERP_MDM_OPERATOR + ERP_SALES_OPERATOR) and 3 active Company-scoped UserRoleAssignments. Per-tenant composite UNIQUE (TenantId, NormalizedName) replacing the legacy GLOBAL UNIQUE RoleNameIndex. Provisioner captures (not throws on) cross-tenant NormalizedName observations in a new `CrossTenantNormalizedNameCollisions` diagnostic field. |
| Out-of-Scope (V1) | SalesOrder UI G1B-1 prototype rebase (3D status model, action matrix, header fields H15-H32); approval workflow UI (Submit / Approve / Reject / Withdraw); closing flow UI (Confirm / Close); multi-Warehouse / Org / Company on the order; 83726107798405120 historical Tenant governance (audit + retention decision); Admin.NET Core changes; AspNetUsers / UserNameIndex / UserRoleAssignment schema changes. |
| Forbidden follow-up without user authorization | Add more roles, modify the 8/12/2 permission claim sets, change the bootstrap snowflake ID constants, modify Admin.NET, modify the migration intra-tenant duplicate preflight, modify the SalesOrder UI before the dedicated rebase goal, push the repository, install psql on Operator side. |
| Next Goal | **`GULIERP_SALES_ORDER_UI_REBASE_001`** (recommended, NOT in this goal scope) — rebuild `apps/web/src/views/sales-order/*.vue` to satisfy the G1B-1 prototype baseline: 3D status model (DocumentStatus / ApprovalStatus / ExecutionStatus), full H1-H32 header fields, action visibility/enablement matrix over the 60-cell state space, approval workflow UI, closing flow UI, multi-Warehouse / Org / Company support, per-line overrides (L18 / L19 / L27). Re-test against `docs/review/G1B1_HARD_FAIL_CHECKLIST.md` and re-issue `GULIERP_SALESORDER_UX_APPROVED`. Discrepancy record: `docs/verification/GULIERP_SALESORDER_UI_BASELINE_DISCREPANCY_NOTE.md`. |
...
| Field | Value |
|---|---|
| Goal | **WEB-PREVIEW-002 — MDM Permission and Company Membership Provisioning** |
| Gate | `WEB_PREVIEW_002_CODE_READY_OPERATOR_PROVISION_PENDING` |
| Status | **Code-side complete** at `cff13e0`. Agent added `--grant-mdm-operator` mode to `tools/GuliERP.Identity.Bootstrap/Program.cs` + `-GrantMdmOperator` switch on `provision-web-preview-user.ps1` + `g2-004-bootstrap-operator-user.ps1`; idempotently grants 12 MDM read+manage permission claims (`mdm.uom.read` ... `mdm.location.manage`) to a Tenant-scoped `ERP_MDM_OPERATOR` role and assigns it to the user. Identity.Bootstrap tests 51/51 (45 prior + 6 new); MDM 67/67, Identity 21/21, Foundation 44/44 no regression. Operator runs `-GrantMdmOperator` once on the canonical PG to upgrade the gate to `WEB_PREVIEW_002_MDM_AUTHORIZATION_VERIFIED`. See `docs/verification/WEB_PREVIEW_002_MDM_AUTHORIZATION_REPORT.md`. |
| Next Goal | **API-CONTRACT-ID-001 — Snowflake/HiLo ID Safe String Wire Contract** (now ACTIVE above) |
...
| Next Goal | **MDM-001 — Real Master Data Vertical Slice** (now CLOSED — see next entry) |
...
| Gate | **`MDM_001_REAL_MASTER_DATA_VERIFIED`** |
| Status | **CLOSED** at 2026-08-21 (R10 manual evidence closure). 122/122 unit + 50/50 integration + idempotent migration + 1 real API start with three 200. See `docs/verification/MDM_001_MANUAL_EVIDENCE_CLOSURE.md`. |
| Entry Gate | `MDM_000_MASTER_DATA_CONVENTION_FROZEN` (frozen) |
| Architecture / Decision | `docs/architecture/MDM_000_MASTER_DATA_CONVENTION_V1.md`; `docs/architecture/TRAE_MDM_001_API_HANDOFF.md`; `docs/governance/GULIERP_GREENFIELD_RISK_REGISTER_V1.md` |
| Verification | Unit 57/57 (final), integration 10/10 (5 rounds × 10), Solution Release build 0/0, API Round 1 PID 1620 + 3x 200, Model Differ clean (R4 probe) |
| Tech Debt | `MDM_ACCEPTANCE_HARNESS_RESUME_BACKFILL_DEFERRED` (R10 closure) — see `MDM_001_MANUAL_EVIDENCE_CLOSURE.md` §2 |
| Next Goal | **MDM-002 — BusinessPartner / Warehouse / Location** (now ACTIVE above) |
...
| Gate | `ID_GENERATION_POSTGRES_HILO_VERIFIED · CLOSED` |
| Status | **CLOSED** — code-side migration to `PRIMARY_TECHNICAL_ID_STRATEGY = POSTGRESQL_HILO_BIGINT` is implemented; first-party Identity technical IDs are `long` / `bigint`; custom Snowflake production path is retired. ID-GEN-001R1 (commit `0463a8d fix(identity): repair missing postgres hilo sequence`) repaired the Operator `42P01` missing sequence failure. R1 also aligned the Operator harness to apply migrations with `--configuration Release --no-build`. Final state: HiLo sequence `identity.gulierp_hilo_sequence` exists; focused real PostgreSQL HiLo tests pass; D-005 (custom Snowflake) is CLOSED. Operator evidence pack recorded in `docs/verification/ID_GEN_001_POSTGRES_HILO_REPORT.md` + `ID_GEN_001R1_MISSING_HILO_SEQUENCE_REPORT.md`. |
| Architecture | `docs/architecture/ID_STRATEGY_FINAL_DECISION.md` (re-evaluated by Codex during ID-GEN-001: `7c908fa docs(architecture): re-evaluate id strategy with postgres hilo`) |
| Next Goal | **MDM-000 — Master Data Convention Freeze** (now ACTIVE above) |
...
The G2003 migration created `gulierp_tenant` AFTER the child
tables that reference it. EF Core's `CreateTable` can only emit
FKs to tables created EARLIER in the same migration. EF Core
should have used `migrationBuilder.AddForeignKey` follow-ups
after all tables existed, but it did not. Result: 12 cross-entity
FKs (TenantId / CompanyId / UserId / RoleId / OrganizationUnitId
chains) were missing from the database. The Application layer
correctly enforced the cross-Tenant invariant; the database was
missing defense-in-depth.

### G2-003V2 — Fix (commit `fa3365a`)

**Strictly additive**. G2003 original migration is NOT modified
(per brief section 5: "禁止修改已经执行过的: G2003 original
migration 来'伪造历史'."). The fix is:

1. `IdentityDbContext.OnModelCreating` declares all 14 missing
   FKs with `OnDelete(DeleteBehavior.Restrict)` (preserves
   DEC-ID-015 soft-delete semantics — no cascading delete of
   historical ERP data).
2. `dotnet ef migrations add G2003V2_AddIdentityReferentialIntegrity`
   generated the additive migration containing 14 `AddForeignKey`
   calls + 9 supporting `CreateIndex` calls. The `Down` migration
   reverses all 17 changes.
3. Model snapshot regenerated automatically.
4. New test file `IdentityReferentialIntegrityFacts` with 3
   Operator-required tests:
   - `FK_Tenant_RejectOnOrphan`: insert Company with bogus
     TenantId → expect `DbUpdateException`
   - `FK_Tenant_AcceptOnValid`: insert Company with seed
     Tenant id → expect success → cleanup
   - `FK_DeleteBehavior_Restrict_TenantCannotBeDeletedWithCompanies`:
     delete seed Tenant → expect `DbUpdateException` (Restrict
     blocks the cascade)

All 14 new FKs use `ReferentialAction.Restrict`. The 6
pre-existing Identity-default FKs (AspNetUserClaims /
UserLogins / UserRoles / UserTokens / RoleClaims) keep their
`Cascade` (Microsoft Identity default; unchanged — clear
ownership semantics for claim tables).

### G2-003V2 — Verification (Mavis side)

| Check | Result |
|---|---|
| `dotnet build GuliERP.slnx -c Release` | 0 warnings / 0 errors across 9 projects |
| `dotnet test GuliERP.slnx -c Release` | 101 PASS / 9 LOUD-FAIL / 0 SKIP |
| 6 G2-001 / G2-003 baseline loud-fails | unchanged |
| 3 new G2-003V2 loud-fails | by design (Operator real-DB required) |
| Forbidden scan | 0 actual uses (2 doc comments in `FoundationDbContext.cs` document the FORBIDDEN list) |
| `git diff --check` | 0 whitespace conflicts |
| G2003 original migration | unchanged (per brief §5) |
| Domain entity classes | unchanged (only EF model FK declarations added) |

### G2-003V2 — G2-003 verification report §20 amendment

The original G2-003 report §20 ("Constraints") listed 12 GuliERP
FKs as "enforced by EF Core mapping" — but the G2003 migration
only contained 3 self-FKs. The other 9+ were never emitted. The
report's claim was overstated. The G2-003V2 commit amends §20
to reflect the actual G2003 baseline (3 self + 6 Identity-default
= 9 FKs) + the G2003V2 amendment (adds 14 cross-entity FKs, total
23 FKs in the `identity` schema after both migrations apply).

This is G2-R0 review finding D-014 (MEDIUM governance defect)
closure. The verification report is now honest about the G2-003
state at the time of the gate flip, and the G2-003V2 closure
record shows how the gap was closed.

### G2-003V2 — G2-R0 review count correction

The original G2-R0 review §1 Executive Decision said "3 HIGH" +
"4 LOW" + "Several INFO". The §18 Final Decision correctly
listed 4 HIGH findings (D-001, D-002, D-003, D-005) + 7 LOW
+ 6 INFO. The G2-003V2 commit corrects §1 to match §18.
This is a documentation hygiene fix per brief section 10.

### G2-003V2 — Hard-stop check (brief §三十九)

| Brief condition | Did G2-003V2 trip it? |
|---|---|
| A. Need to change DEC-ID-001..020 | NO (20/20 preserved; this Goal CLOSES FK enforcement, not DEC-IDs) |
| B. Plant/Company/Organization boundary conflict | NO |
| C. Pre-implement Permission | NO |
| D. Pre-implement JWT/Auth | NO |
...
# tests step will run the 3 new tests; the runtime rounds +
# bad-DB negative round are unchanged).

# Then edit this file (docs/governance/GOAL_REGISTRY.md):
#   - flip the Active Goal table from
#       G2_003V2_CODE_READY_OPERATOR_DB_RERUN_PENDING
#     to
#       G2_003V2_IDENTITY_REFERENTIAL_INTEGRITY_VERIFIED
#   - add Operator-verified date

# Commit the flip.
git add docs/governance/GOAL_REGISTRY.md
git commit -m "docs(verification): operator-upgrade G2-003V2 to IDENTITY_REFERENTIAL_INTEGRITY_VERIFIED"
```
...
STARTED, HALTED). Hard stop: G2-004 must NOT auto-start in
