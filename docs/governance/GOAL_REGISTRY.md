...
| Status | **CODE_READY + STEP4B_REPAIRED + RELATIONSHIPS_ALIGNED + OPERATOR_RERUN_PENDING** — 3-project MDM module scaffolded (Domain / Application / Infrastructure) and wired into the API host. 12 HTTP endpoints under `/api/v1/mdm/{uoms,item-categories,items}` mapped. Code canonicalization (trim + uppercase) + unique constraints applied at DB layer. ItemCategory self-FK cycle detection in service layer. Cross-tenant returns 404 (not 403) to avoid leaking resource existence. Foundation `ICurrentTenant` / `ICurrentUser` reused. PostgreSQL HiLo reuses `identity.gulierp_hilo_sequence` (ID-GEN-001). Migration `20260820190000_MDM001_InitializeMdmSchema` creates `mdm` schema + 3 tables + indexes + FKs. `MdmSeed` loads 13 DEV `SAFE_TO_SEED_SYSTEM` UOM rows from `data/bootstrap/reference/system/uom.json` (idempotent). **Code-side evidence PASS** (build 0/0, MDM 43/43, Identity 21/21, Foundation 44/44, Integration 10 discovered). **MDM-001R4 fixed** (HasOne anonymous → HasOne(navigation) + 4 FK indexes explicit; EF Core 10 convention detector was creating 3 shadow FKs `*Id1`; runtime + Snapshot fully aligned, 0 pending model changes verified via `dotnet ef migrations add _R4_Probe2` dry-run, EF generated no corrective migration). **MDM-001R3 fixed** (Designer.cs:234 + Snapshot.cs:249 — `HasIndex("Code", "ux_…")` 2-arg form was removed in EF Core 7+, replaced with chained `HasIndex("Code").HasDatabaseName("ux_…")`). **MDM-001R2 fixed** (Step 4a false negative — PowerShell array `-notmatch` gotcha). **Real PG verification PENDING** — Operator must run `tools/dev/mdm-001-operator-evidence.ps1` in own terminal per `docs/verification/MDM_001_RELATIONSHIP_MODEL_ALIGNMENT_FIX_REPORT.md` §13 to upgrade gate to `MDM_001_REAL_MASTER_DATA_VERIFIED`. Closure: `MDM_001_OPERATOR_EVIDENCE_CLOSURE_REPORT.md` → R2: `MDM_001_MIGRATION_DISCOVERY_HARNESS_FIX_REPORT.md` → R3: `MDM_001_MIGRATION_SNAPSHOT_INDEX_FIX_REPORT.md` → R4: `MDM_001_RELATIONSHIP_MODEL_ALIGNMENT_FIX_REPORT.md`. |
...
| Local Evidence | `dotnet build GuliERP.slnx -c Release` → 22 projects, 0 warnings / 0 errors (6.78s R4, 24.88s R3, 11.13s R1). `dotnet test tests/GuliERP.Mdm.Tests` → **43/43 PASS** (R4 +12, R3 +7, R1 24). `dotnet test tests/GuliERP.Identity.Tests` → 21/21 PASS (no regression). `dotnet test tests/GuliERP.Foundation.Tests` → 44/44 PASS (no regression). `dotnet test tests/GuliERP.Mdm.IntegrationTests --list-tests` → 10 tests, EXIT 0. `dotnet ef migrations list` → EXIT 0, lists `20260820190000_MDM001_InitializeMdmSchema (Pending)`. `dotnet ef migrations add _R4_Probe2` → produced no corrective migration (proves 0 pending model changes). Operator harness `tools/dev/mdm-001-operator-evidence.ps1` present (R2 fixed array -notmatch gotcha); NOT executed in agent session due to PGPASSWORD env block. |
...
| Next Mainline | **MDM-001 Operator Evidence** (real PG runs `tools/dev/mdm-001-operator-evidence.ps1` to upgrade gate from `MDM_001_OPERATOR_EVIDENCE_HARD_STOP` to `MDM_001_REAL_MASTER_DATA_VERIFIED`; safe PowerShell command template in `docs/verification/MDM_001_RELATIONSHIP_MODEL_ALIGNMENT_FIX_REPORT.md` §13). After Operator evidence, NEXT MAINLINE is `MDM-002 — BusinessPartner / Warehouse / Location` (BusinessPartner 3 confirmed roles, 4th role DEFERRED; Warehouse has OPTIONAL PlantId; Location belongs to Warehouse). |
