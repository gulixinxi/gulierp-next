...
| Harness | `tools/dev/g2-004-operator-evidence.ps1` (rewritten; concurrent pipe drain + 60s bootstrap timeout + `$script:BaselineAtG2_004=174` minimum gate; user-visible banner unified to `G2-004`) + `tools/dev/g2-004-bootstrap-operator-user.ps1` (concurrent pipe drain + 60s timeout + script-owned PID kill on timeout) + `tools/GuliERP.Identity.Bootstrap/StderrLoggerProvider.cs` (custom `ILoggerProvider` writing to stderr; stdout reserved for final JSON) + `tests/GuliERP.Identity.Bootstrap.Tests/ProcessIoDeadlockFacts.cs` (2 new tests: concurrent drain + timeout-kill) |
