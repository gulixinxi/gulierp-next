...
| Gate | `GULIERP_ENTERPRISE_BOOTSTRAP_001_BUSINESS_ROLE_PACK_VERIFIED` |
| Status | **CLOSED at 2026-08-23** — Operator Apply confirmed: `ok=true`, `tenantId=83727350616817890`, `companyId=83727350616817891`, `userId=83727350616817894`, `mdmRoleCreated=true`, `salesRoleCreated=true`, `mdmClaimsCreated=12`, `salesClaimsCreated=2`, `mdmAssignmentCreated=true`, `salesAssignmentCreated=true`, `rolePackStatus=FORMAL_ENTERPRISE_BUSINESS_ROLE_PACK_APPLIED`. The Formal Enterprise Bootstrap foundation is now end-to-end functional for `GULI` / `GULI001` / `admin`. Schema Repair (3 commits, ending at `70fd5d2`) converted the legacy ASP.NET Identity default GLOBAL UNIQUE `identity."AspNetRoles"."RoleNameIndex"` (NormalizeName) into a per-tenant composite UNIQUE `ux_gulierp_role_tenant_normalizedname (TenantId, NormalizedName)` via EF migration `20260823090247_RoleNameIndexToTenantScope` (Step 0 intra-tenant duplicate preflight + Step 1 DROP `RoleNameIndex` + Step 2 NULL/empty backfill + Step 3 CREATE composite UNIQUE, all in one PG transaction). The 23505 `RoleNameIndex` cross-tenant NormalizedName collision that blocked the original `--ensure-formal-enterprise-business-role-pack GULI GULI001 admin` Apply is now structurally impossible. 7/7 test suites + 3 new Provisioner diagnostic tests = 225/225 PASS at `867ed4d`; zero Admin.NET / zero AspNetUsers / zero business data changes. Operator runbook `docs/verification/GULIERP_ROLE_TENANT_ISOLATION_OPERATOR_DB_UPGRADE_RUNBOOK.md` records the Apply path. Next: `GULIERP_ENTERPRISE_BOOTSTRAP_001_RUNTIME_VERIFIED` (formal admin logout/relogin + MDM/Sales browser Runtime) — NOT in this goal's scope, deferred to the Operator UI validation phase. |
...
| Out-of-Scope (V1) | Runtime UI validation (formal admin logout/relogin + MDM/Sales browser); 83726107798405120 historical Tenant治理 (audit + retention decision); Admin.NET Core changes; AspNetUsers / UserNameIndex / UserRoleAssignment schema changes. |
| Forbidden follow-up without user authorization | Add more roles, modify the 8/12/2 permission claim sets, change the bootstrap snowflake ID constants, modify Admin.NET, modify the migration's intra-tenant duplicate preflight, push the repository, install psql on Operator side. |
| Next Goal | **GULIERP-ENTERPRISE-BOOTSTRAP-001-RUNTIME — Formal Admin Browser Runtime Validation** (deferred until Operator triggers it) |
