...
| Non-blocking Technical Debt (closed under MDM-001) | `MDM_ACCEPTANCE_HARNESS_RESUME_BACKFILL_DEFERRED` — API auto Stop/Restart Round 2 未形成最终机器证据;R7 已有一次真实 API 启动 + 三个 200;R8 + R9 修复了 Harness(`-ResumeApiRuntime` + Operator Transcript Backfill)但本会话未在 Operator 终端重跑;**不影响 MDM 业务模块正确性**(122/122 unit + 50/50 integration + idempotent migration + 1 real API start with three 200),留待跨模块验收 Harness 统一治理。详细原因 / 后续 Goal 范围见 `docs/verification/MDM_001_MANUAL_EVIDENCE_CLOSURE.md` §2。 |
