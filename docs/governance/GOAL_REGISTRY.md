...
| Field | Value |
|---|---|
| Goal | **G2-004V1R4 — Bootstrap Process Pipe Deadlock Fix** (1 high-probability root cause: classic redirected-pipe deadlock between parent and the stderr-flooding .NET bootstrap tool; tooling only) |
| Gate | `G2_004_OPERATOR_EVIDENCE_HARNESS_R3_READY` (G2-004 + G2-004R1 + G2-004V1 + G2-004V1R1 + G2-004V1R2 + G2-004V1R3 closed) |
| Status | **`G2_004_OPERATOR_EVIDENCE_HARNESS_R4_READY`** — Mavis-side 164/173 tests PASS / 9 LOUD-FAIL (+2 new ProcessIoDeadlockFacts tests; baseline 171 → 173 after arithmetic correction from V1R4's off-by-one 174). 0 production code change. 1 root cause fixed: bootstrap process IO deadlock (concurrent `ReadToEndAsync` on stdout + stderr + 60s `WaitForExit` timeout + script-owned PID kill on timeout). User-visible banner unified to stable `G2-004`. Operator unlocks to `G2_004_AUTHENTICATION_KERNEL_VERIFIED` after the 8-step evidence run with the new harness. |
| Entry Gate | `G2_004_OPERATOR_EVIDENCE_HARNESS_R3_READY` (G2-004 + G2-004R1 + G2-004V1 + G2-004V1R1 + G2-004V1R2 + G2-004V1R3 closed) |
| Verification | `docs/verification/G2_004V1R4_PROCESS_IO_DEADLOCK_REPORT.md` |
| Harness | `tools/dev/g2-004-operator-evidence.ps1` (rewritten; concurrent pipe drain + 60s bootstrap timeout + `$script:BaselineAtG2_004=173` minimum gate; user-visible banner unified to `G2-004`) + `tools/dev/g2-004-bootstrap-operator-user.ps1` (concurrent pipe drain + 60s timeout + script-owned PID kill on timeout) + `tools/GuliERP.Identity.Bootstrap/StderrLoggerProvider.cs` (custom `ILoggerProvider` writing to stderr; stdout reserved for final JSON) + `tests/GuliERP.Identity.Bootstrap.Tests/ProcessIoDeadlockFacts.cs` (2 new tests: concurrent drain + timeout-kill) |
