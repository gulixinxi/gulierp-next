...
| Next Mainline | **MDM-002 Operator Runtime Acceptance** (Operator runs the integration tests in `tests/GuliERP.Mdm.IntegrationTests/MdmBusinessPartnerWarehouseLocationFacts.cs` against real PG, plus optionally hits the 12 new HTTP endpoints with curl / Postman; password via `Read-Host -AsSecureString`; 2 new integration tests must PASS + 10 existing MDM-001 integration tests must remain GREEN). After Operator evidence, NEXT MAINLINE is **`MDM-003 — TRAE MDM-002 Web Wiring`**: UOM/Item/ItemCategory pages already wired in MDM-WEB-001; BusinessPartner/Warehouse/Location front-end wiring follows the same `apps/web/src/api/mdm/*.ts` + Cookie + CSRF + RFC7807 pattern. |
| Forbidden follow-up without user authorization | WorkCenter implementation, Coding Engine implementation, Precision / Rounding implementation, Document Numbering implementation, Inventory / Sales / Purchase business API implementation, MDM-000 re-freeze, MDM-002 front-end wiring (deferred to MDM-003), SalesOrder / PurchaseOrder / Inventory business API, Approval Workflow, SalesOrder Mock UX changes. |
| Non-blocking Technical Debt (closed under MDM-002) | `MDM_002_INTEGRATION_RUNTIME_DEFERRED` — 12 integration tests (10 existing + 2 new MDM-002) are Operator-required against real PG. Agent session has no PG (no Docker / Testcontainers / local PG / canonical password) → `dotnet test .../GuliERP.Mdm.IntegrationTests` cannot be executed in agent session. **Does NOT block MDM-002 code-side gate** because: (a) all 67 MDM unit tests PASS, (b) Identity 21/21 + Foundation 44/44 no regression, (c) Model Differ clean (`_MDM002_Probe` migration has 0 operations), (d) MDM-001 migration untouched (`git diff` clean), (e) Solution Release build 0/0, (f) Service Boundary tests enforce Tenant/Company scope at source level. Operator runs integration tests in own terminal per `docs/architecture/TRAE_MDM_002_API_HANDOFF.md` §14. |
