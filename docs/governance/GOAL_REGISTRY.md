...
| Gate | **`MDM_001_OPERATOR_EVIDENCE_HARD_STOP`** |
| Status | **CODE_READY + STEP4B_REPAIRED + OPERATOR_RERUN_PENDING** — 3-project MDM module scaffolded (Domain / Application / Infrastructure) and wired into the API host. 12 HTTP endpoints under `/api/v1/mdm/{uoms,item-categories,items}` mapped. Code canonicalization (trim + uppercase) + unique constraints applied at DB layer. ItemCategory self-FK cycle detection in service layer. Cross-tenant returns 404 (not 403) to avoid leaking resource existence. Foundation `ICurrentTenant` / `ICurrentUser` reused. PostgreSQL HiLo reuses `identity.gulierp_hilo_sequence` (ID-GEN-001). Migration `20260820190000_MDM001_InitializeMdmSchema` creates `mdm` schema + 3 tables + indexes + FKs. `MdmSeed` loads 13 DEV `SAFE_TO_SEED_SYSTEM` UOM rows from `data/bootstrap/reference/system/uom.json` (idempotent). **Code-side evidence PASS** (build 0/0, MDM 31/31, Identity 21/21, Foundation 44/44, Integration 10 discovered, **R3 Snapshot BuildModel regression test PASS**). **MDM-001R3 fixed** (Designer.cs:234 + Snapshot.cs:249 — `HasIndex("Code", "ux_…")` 2-arg form was removed in EF Core 7+, replaced with chained `HasIndex("Code").HasDatabaseName("ux_…")`). **MDM-001R2 fixed** (Step 4a false negative — PowerShell array `-notmatch` gotcha; see `MDM_001_MIGRATION_DISCOVERY_HARNESS_FIX_REPORT.md`). **Real PG verification PENDING** — Operator must run `tools/dev/mdm-001-operator-evidence.ps1` in own terminal per `docs/verification/MDM_001_MIGRATION_SNAPSHOT_INDEX_FIX_REPORT.md` §11 to upgrade gate to `MDM_001_REAL_MASTER_DATA_VERIFIED`. Closure: `MDM_001_OPERATOR_EVIDENCE_CLOSURE_REPORT.md` → R2 fix: `MDM_001_MIGRATION_DISCOVERY_HARNESS_FIX_REPORT.md` → R3 fix: `MDM_001_MIGRATION_SNAPSHOT_INDEX_FIX_REPORT.md`. |
...
| Local Evidence | `dotnet build GuliERP.slnx -c Release` → 22 projects, 0 warnings / 0 errors (24.88s R3, 11.13s R1). `dotnet test tests/GuliERP.Mdm.Tests` → **31/31 PASS** (R3 +7, R1 24). `dotnet test tests/GuliERP.Identity.Tests` → 21/21 PASS (no regression). `dotnet test tests/GuliERP.Foundation.Tests` → 44/44 PASS (no regression). `dotnet test tests/GuliERP.Mdm.IntegrationTests --list-tests` → 10 tests, EXIT 0. `dotnet ef migrations list` → EXIT 0, lists `20260820190000_MDM001_InitializeMdmSchema (Pending)` (R3 verified post-fix; pre-fix would throw `InvalidOperationException`). Operator harness `tools/dev/mdm-001-operator-evidence.ps1` present (R2 fixed array -notmatch gotcha); NOT executed in agent session due to PGPASSWORD env block. |
...
| Next Mainline | **MDM-001 Operator Evidence** (real PG runs `tools/dev/mdm-001-operator-evidence.ps1` to upgrade gate from `MDM_001_OPERATOR_EVIDENCE_HARD_STOP` to `MDM_001_REAL_MASTER_DATA_VERIFIED`; safe PowerShell command template in `docs/verification/MDM_001_MIGRATION_SNAPSHOT_INDEX_FIX_REPORT.md` §11). After Operator evidence, NEXT MAINLINE is `MDM-002 — BusinessPartner / Warehouse / Location` (BusinessPartner 3 confirmed roles, 4th role DEFERRED; Warehouse has OPTIONAL PlantId; Location belongs to Warehouse). |
